...
# 5-hand evaluator first attempt. 
#(9724 hands per second)
#1 million hands in 121 seconds.
def evalHand1(hand):
...
# Evaluates 7 cards (generating all possible combinations) and returns the highest one.
# Used for debuggin purposes
# 1117 hands per second
def getBestOfSeven(cards):
    sets = itertools.combinations(cards, 5)
    combs = []
    for s in sets:
       if evalHand2(s) not in combs: combs.append(evalHand2(s))
    bestOpt = h.HIGH_CARD
    for c in combs:
        if (c.value > bestOpt.value): bestOpt = c
    return bestOpt
